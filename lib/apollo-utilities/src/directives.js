...
function getDirectiveInfoFromField(field, variables) {
    if (field.directives && field.directives.length) {
        ...
        field.directives.forEach(function (directive) {
            directiveObj[directive.name.value] = storeUtils_1.argumentsObjectFromField(directive, variables);
        });
        return directiveObj;
    }
    return null;
...
function shouldInclude(selection, variables) {
    ...
    return getInclusionDirectives(selection.directives).every(function (_a) {
        ...
        if (ifArgument.value.kind === 'Variable') {
            evaledValue = variables[(ifArgument.value)];
            as;
            graphql_1.VariableNode;
        }
    }).name.value;
    ;
...
{
    evaledValue = (ifArgument.value);
    as;
    graphql_1.BooleanValueNode;
    value;
}
